...
1. **AES-GCM 256-bit Key Isolation:** Encryption keys are dynamically generated utilizing the native Web Crypto API (`window.crypto.subtle`). Keys are *never* transmitted to the `humanos.foundation` backend.
