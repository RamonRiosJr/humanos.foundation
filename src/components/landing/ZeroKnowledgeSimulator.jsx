...
export default function ZeroKnowledgeSimulator() {
    ...
    const [rawText, setRawText] = useState(
        "Patient reports onset of acute migraines localized to the frontal lobe, accompanied by photophobia and mild nausea. Heart rate elevated at 92 BPM.",
    );
            ...
            <div className="max-w-6xl mx-auto">
                <div className="text-center mb-16 md:mb-24">
                    ...
                    <p className="text-white/50 max-w-2xl mx-auto mt-6 text-sm md:text-base leading-relaxed">
                        We mathematically guarantee your privacy by encrypting
                        your data locally on your device before it ever touches
                        the network. Our cloud databases are completely blind to
                        your actual health narrative.
                    </p>
                </div>
                ...
                <div className="grid md:grid-cols-2 gap-8 lg:gap-12 items-center">
                    ...
                    <div className="glass rounded-2xl border border-white/10 overflow-hidden relative shadow-2xl">
                        ...
                        <div className="bg-black/40 px-4 py-3 border-b border-white/5 flex items-center gap-2">
                            ...
                            <span className="text-[10px] text-white/30 ml-2 font-mono tracking-wider">
                                Aura_Local_Engine_v0.8
                            </span>
                        </div>
                        ...
                        <div className="p-6 md:p-8">
                            {inputMode ? (
                                ...
                                <div className="space-y-6 h-[236px] flex flex-col justify-center">
                                    ...
                                    >
                                        <div
                                            className={`p-2 rounded-lg ${encryptionStage >= 1 ? "bg-emerald-500/20 text-emerald-400" : "bg-white/5 text-white/20"}`}
                                        >
                                            <KeyRound className="w-5 h-5" />
                                        </div>
                                        <div>
                                            <p className="text-sm font-bold text-white/90">
                                                Deriving Local Device Key
                                            </p>
                                        ...
                                        {encryptionStage === 1 && (
                                            <motion.div
                                                ...
                                                className="w-4 h-4 border-2 border-emerald-500/30 border-t-emerald-400 rounded-full ml-auto"
                                            />
                                        )}
                                        {encryptionStage > 1 && (
                                            <ShieldCheck className="w-5 h-5 text-emerald-400 ml-auto" />
                                        )}
                                    </motion.div>
                                    ...
                                    <AnimatePresence>
                                        {encryptionStage >= 2 && (
                                            ...
                                            >
                                                <div
                                                    className={`p-2 rounded-lg ${encryptionStage >= 2 ? "bg-cyan-500/20 text-cyan-400" : "bg-white/5 text-white/20"}`}
                                                >
                                                    <Cpu className="w-5 h-5" />
                                                </div>
                                                <div>
                                                    <p className="text-sm font-bold text-white/90">
                                                        Applying AES-GCM 256
                                                        Cipher
                                                    </p>
                                                    <p className="text-xs text-white/40 font-mono">
                                                ...
                                                {encryptionStage === 2 && (
                                                    <motion.div
                                                        ...
                                                        className="w-4 h-4 border-2 border-cyan-500/30 border-t-cyan-400 rounded-full ml-auto"
                                                    />
                                                )}
                                                {encryptionStage > 2 && (
                                                    <ShieldCheck className="w-5 h-5 text-cyan-400 ml-auto" />
                                                )}
                                            </motion.div>
                                        )}
                                    </AnimatePresence>
                                ...
                                {encryptionStage < 3 ? (
                                    ...
                                    >
                                        <div className="flex justify-between text-white/40 mb-3 pb-3 border-b border-white/5">
                                            ...
                                            <span>
                                                PAYLOAD: {cipherText.length}B
                                            </span>
                                        </div>
                                        <p
                                            className="text-emerald-400/80 break-all leading-relaxed whitespace-pre-wrap h-32 overflow-y-auto pr-2"
                                            style={{
                                                scrollbarWidth: "thin",
                                                scrollbarColor:
                                                    "#34d399 transparent",
