...
const ReadingMask = () => {
    ...
    useEffect(() => {
        ...
        const handleMouseMove = (e) =>
            requestAnimationFrame(() => setMouseY(e.clientY));
        ...
        <>
            ...
            <div className="fixed bottom-6 left-6 z-[9990] transition-all duration-300">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-[10px] text-blue-400 font-mono tracking-widest uppercase">
                                        Aura hOS Engine Active
                                    </p>
                                </div>
                            </div>
                            <button
                                onClick={() => setIsOpen(false)}
                                className="text-slate-400 hover:text-white transition-transform hover:rotate-90 bg-white/5 p-1.5 rounded-full"
                                aria-label="Close"
                            >
                                <X className="w-4 h-4" />
                            </button>
                ...
                <button
                    ...
                    className={`relative w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-500 border overflow-hidden group shadow-[0_0_20px_rgba(59,130,246,0.3)] ${
                        isOpen
                            ? "bg-blue-600 border-blue-400 text-white shadow-[0_0_30px_rgba(59,130,246,0.6)] scale-95"
                            : "bg-slate-900 border-white/20 text-blue-400 shadow-xl hover:border-blue-500/50 hover:bg-slate-800 hover:-translate-y-1"
                    }`}
                ...
                >
                    ...
                    {isOpen ? (
                        ...
                        <Accessibility className="w-6 h-6 relative z-10 transition-transform group-hover:scale-110" />
                    )}
                </button>
            </div>
        </>
...
const A11yToggle = ({ active, onClick, icon, label, description }) => {
    return (
        <button
            onClick={onClick}
            className={`w-full flex items-center justify-between p-3 rounded-xl border transition-all duration-300 group text-left ${
                active
        ...
        >
            <div className="flex gap-3">
                <div
                    className={`mt-0.5 transition-colors ${active ? "text-blue-400" : "text-slate-400 group-hover:text-blue-300"}`}
                >
                    {icon}
                </div>
                <div>
                    <span
                        className={`block text-sm font-bold tracking-wide transition-colors ${active ? "text-blue-100" : "text-slate-200"}`}
                    >
                        {label}
                    </span>
            ...
            <div
                className={`w-9 h-5 rounded-full transition-colors relative flex-shrink-0 ${active ? "bg-blue-500" : "bg-slate-700/50 border border-slate-600"}`}
            >
                <div
                    className={`absolute top-[2px] left-[2px] w-4 h-4 bg-white rounded-full transition-transform shadow-sm ${active ? "translate-x-4" : "translate-x-0"}`}
                />
            </div>
        </button>
    );
