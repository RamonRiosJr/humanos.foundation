                ...
                <div className="max-w-5xl mx-auto space-y-14">
                    ...
                    <div className="grid sm:grid-cols-3 gap-5">
                        {upcomingTypes.map((type, i) => (
                            <motion.div key={type.label} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1 }} className="glass rounded-2xl p-7 border border-foreground/[0.04] hover:border-cyan-500/20 transition-all">
                                <type.icon className="w-6 h-6 text-cyan-400/50 mb-4" />
                                <h3 className="text-base font-semibold text-foreground/70 mb-2" style={{ fontFamily: 'Outfit, Inter, sans-serif' }}>{type.label}</h3>
                                <p className="text-xs text-foreground/35 leading-relaxed">{type.desc}</p>
                            </motion.div>
                        ))}
                    </div>

                    {/* Coming soon */}
                    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center glass-strong rounded-3xl p-12 md:p-16 border border-foreground/[0.04]">
                        <Calendar className="w-12 h-12 text-foreground/20 mx-auto mb-5" />
                        <h2 className="text-xl font-semibold text-foreground/50 mb-3" style={{ fontFamily: 'Outfit, Inter, sans-serif' }}>Events Calendar Coming Soon</h2>
                        <p className="text-sm text-foreground/25 max-w-md mx-auto mb-7">
