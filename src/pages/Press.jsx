                ...
                <div className="max-w-5xl mx-auto space-y-10">
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="glass-strong rounded-3xl p-8 md:p-12 border border-cyan-500/10">
                        <div className="flex items-center gap-3 mb-5">
                            ...
                            <h2 className="text-xl font-bold text-foreground/80" style={{ fontFamily: 'Outfit, Inter, sans-serif' }}>Press Contact</h2>
                        </div>
                        <div className="grid sm:grid-cols-2 gap-6">
                            <div>
                                <p className="text-sm text-foreground/50 mb-1">Media Inquiries</p>
                            ...
                            <div>
                                <p className="text-sm text-foreground/50 mb-1">Founder</p>
                            ...
                            <div>
                                <p className="text-sm text-foreground/50 mb-1">Response Time</p>
                                <p className="text-sm text-foreground/35">Within 24 hours for press inquiries</p>
                            </div>
                            <div>
                                <p className="text-sm text-foreground/50 mb-1">Organization</p>
                    ...
                    {/* Boilerplate */}
                    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="glass rounded-3xl p-8 md:p-10 border border-foreground/[0.04]">
                        <div className="flex items-center gap-3 mb-5">
                            <Quote className="w-5 h-5 text-cyan-400/60" />
                            <h2 className="text-lg font-bold text-foreground/70" style={{ fontFamily: 'Outfit, Inter, sans-serif' }}>Official Boilerplate</h2>
                        </div>
                        <p className="text-sm text-foreground/40 leading-relaxed italic border-l-2 border-cyan-500/20 pl-5">{boilerplate}</p>
                        <button
                            onClick={() => navigator.clipboard.writeText(boilerplate)}
                            className="mt-5 px-4 py-2 rounded-xl glass border border-foreground/[0.06] text-xs text-foreground/30 hover:text-cyan-400 hover:border-cyan-500/20 transition-all flex items-center gap-2"
                        >
                    ...
                    {/* Key topics */}
                    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.15 }} className="glass rounded-3xl p-8 md:p-10 border border-foreground/[0.04]">
                        <div className="flex items-center gap-3 mb-6">
                            <Newspaper className="w-5 h-5 text-cyan-400/60" />
                            <h2 className="text-lg font-bold text-foreground/70" style={{ fontFamily: 'Outfit, Inter, sans-serif' }}>Story Angles We Support</h2>
                        </div>
                        <ul className="space-y-3">
                            {[
                                ...
                                <li key={i} className="flex items-start gap-3 text-sm text-foreground/40">
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }}>
                        <h2 className="text-lg font-bold text-foreground/60 mb-5" style={{ fontFamily: 'Outfit, Inter, sans-serif' }}>Credible External Resources</h2>
                        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            {pressLinks.map((link) => (
                                <a key={link.label} href={link.url} target="_blank" rel="noopener noreferrer" className="glass rounded-2xl p-5 border border-foreground/[0.04] hover:border-cyan-500/20 transition-all group">
                                    <div className="flex items-start justify-between">
                                        <p className="text-sm font-medium text-foreground/60 group-hover:text-foreground/80 transition-colors">{link.label}</p>
                                        <ExternalLink className="w-3.5 h-3.5 text-foreground/20 group-hover:text-cyan-400/60 transition-colors shrink-0 mt-0.5" />
                                    </div>
                                    <p className="text-xs text-foreground/25 mt-1">{link.desc}</p>
                                </a>
                            ))}
                        </div>
                    </motion.div>
