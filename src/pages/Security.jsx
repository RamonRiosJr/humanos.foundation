            ...
            <section className="pb-28 px-4 md:px-8">
                <div className="max-w-4xl mx-auto space-y-8">
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="glass-strong rounded-3xl p-8 md:p-12 border border-cyan-500/10 text-center">
                        ...
                        <h2 className="text-xl font-bold text-foreground/70 mb-3" style={{ fontFamily: 'Outfit, Inter, sans-serif' }}>Found a Vulnerability?</h2>
                        <p className="text-sm text-foreground/35 mb-6 max-w-md mx-auto">Please report it responsibly. We appreciate the security community's work in keeping our systems safe.</p>
                        <a href="mailto:security@humanos.foundation" className="inline-flex items-center gap-2 glow-btn px-6 py-3 rounded-xl bg-gradient-to-r from-cyan-500/20 to-blue-500/20 border border-cyan-500/30 text-sm text-foreground font-medium">
                            <Mail className="w-4 h-4" /> security@humanos.foundation
                        </a>
                    </motion.div>

                    {/* Our commitments */}
                    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="glass rounded-3xl p-8 border border-foreground/[0.04]">
                        <div className="flex items-center gap-3 mb-6">
                            <CheckCircle2 className="w-5 h-5 text-green-400/60" />
                            <h3 className="text-base font-semibold text-foreground/60" style={{ fontFamily: 'Outfit, Inter, sans-serif' }}>Our Commitments to You</h3>
                        </div>
                        <ul className="space-y-3">
                            {commitments.map((c, i) => (
                                <li key={i} className="flex items-start gap-3">
                                    <CheckCircle2 className="w-3.5 h-3.5 text-green-400/40 shrink-0 mt-0.5" />
                                    <p className="text-sm text-foreground/40">{c}</p>
                                </li>
                            ))}
                        </ul>
                    </motion.div>
                    ...
                    <div className="grid sm:grid-cols-2 gap-6">
                        {/* In scope */}
                        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.15 }} className="glass rounded-2xl p-6 border border-foreground/[0.04]">
                            <p className="text-xs text-cyan-400/50 uppercase tracking-widest mb-4">In Scope</p>
                            <ul className="space-y-2">
                                {inScope.map((item, i) => (
                                    <li key={i} className="flex items-start gap-2 text-xs text-foreground/40">
                        ...
                        {/* Out of scope */}
                        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }} className="glass rounded-2xl p-6 border border-foreground/[0.04]">
