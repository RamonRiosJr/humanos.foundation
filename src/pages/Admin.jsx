        ...
        <div className="bg-obsidian min-h-screen text-white">
            ...
            <div className="sticky top-0 z-40 border-b border-white/[0.05] bg-obsidian/90 backdrop-blur-xl px-6 py-4 flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                    <a
                        ...
                        className="flex items-center gap-1.5 text-[11px] text-white/30 hover:text-cyan-400 transition-colors"
                    >
            ...
            <div className="max-w-6xl mx-auto px-6 py-10 space-y-14">
                ...
                <section>
                    ...
                    <div className="grid md:grid-cols-3 gap-6">
                        ...
                        <div className="glass rounded-2xl p-5 border border-white/[0.04]">
                            <p className="text-[10px] text-white/30 uppercase tracking-widest font-medium mb-4">By Role</p>
                        ...
                        {/* Recent signups */}
                        <div className="md:col-span-2 glass rounded-2xl border border-white/[0.04] overflow-hidden">
                            <div className="px-5 py-4 border-b border-white/[0.04]">
                                <p className="text-[10px] text-white/30 uppercase tracking-widest font-medium">Recent Signups</p>
                            </div>
                                ...
                                {joinRequests.slice(0, 20).map((r) => (
                                    <div key={r.id} className="px-5 py-3 flex items-start gap-3">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="text-xs text-white/70 font-medium truncate">{r.name}</div>
                                            <div className="text-[10px] text-white/25 truncate">{r.email}</div>
                                        </div>
                                        <div className="text-[10px] text-cyan-400/50 capitalize whitespace-nowrap">{r.role}</div>
                                    </div>
                                ))}
                                {joinRequests.length === 0 && (
                                    <div className="px-5 py-8 text-center text-xs text-white/20">No signups yet.</div>
                                )}
                            </div>
                        </div>
                    </div>
                ...
                <section>
                    ...
                    <div className="grid md:grid-cols-3 gap-6">
                        <div className="glass rounded-2xl p-5 border border-white/[0.04]">
                            <p className="text-[10px] text-white/30 uppercase tracking-widest font-medium mb-4">By Inquiry Type</p>
                            {Object.entries(byInquiry).sort(([, a], [, b]) => b - a).map(([type, count]) => (
                                <div key={type} className="flex items-center justify-between py-2 border-b border-white/[0.03] last:border-0">
                                    <span className="text-xs text-white/50 capitalize">{type}</span>
                                    <span className="text-xs font-bold text-yellow-400">{count}</span>
                                </div>
                            ))}
                            {contacts.length === 0 && <p className="text-xs text-white/20 mt-2">No messages yet.</p>}
                        </div>
                        <div className="md:col-span-2 glass rounded-2xl border border-white/[0.04] overflow-hidden">
                            <div className="px-5 py-4 border-b border-white/[0.04]">
                                <p className="text-[10px] text-white/30 uppercase tracking-widest font-medium">Recent Messages</p>
                            </div>
                            <div className="divide-y divide-white/[0.03] max-h-80 overflow-y-auto">
                                {contacts.slice(0, 15).map((c) => (
                                    <div key={c.id} className="px-5 py-3">
                                        ...
                                        <div className="text-[10px] text-white/35 truncate">{c.subject || c.message}</div>
                                ...
                                {contacts.length === 0 && (
                                    <div className="px-5 py-8 text-center text-xs text-white/20">No messages yet.</div>
                                )}
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="divide-y divide-white/[0.03]">
                            {posts.slice(0, 10).map((p) => (
                                <div key={p.id} className="px-5 py-3.5 flex items-center gap-4">
                                    <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${p.published ? 'bg-green-400' : 'bg-white/20'}`} />
                                    <div className="flex-1 min-w-0">
                                        <div className="text-xs text-white/70 font-medium truncate">{p.title}</div>
                                        <div className="text-[10px] text-white/25">{p.category} · {p.read_time_minutes || '?'} min read</div>
                                    </div>
                                    {p.featured && <span className="text-[9px] text-cyan-400 border border-cyan-500/20 px-2 py-0.5 rounded-full">Featured</span>}
                                    <span className={`text-[9px] px-2 py-0.5 rounded-full ${p.published ? 'bg-green-500/10 text-green-400' : 'bg-white/5 text-white/25'}`}>
                                        {p.published ? 'Live' : 'Draft'}
                                    </span>
                                </div>
                            ))}
                            {posts.length === 0 && <div className="px-5 py-8 text-center text-xs text-white/20">No posts yet.</div>}
                        </div>
                    </div>
                </section>
                ...
                <section>
                            ...
                            return (
                                <div key={title} className={`rounded-2xl border p-5 ${colors[color].split(' ').slice(0, 2).join(' ')}`}>
                                    <div className="flex items-start gap-3 mb-3">
                                        ...
                                        <div>
                                            <div className="text-xs font-semibold text-white/80 leading-tight">{title}</div>
                                            <span className={`text-[9px] ${colors[color].split(' ').pop()} mt-0.5 inline-block`}>{status}</span>
                                        </div>
                                    </div>
                                    <p className="text-[11px] text-white/35 mb-3 leading-relaxed">{desc}</p>
                    ...
                    <div className="mt-5 glass rounded-2xl p-5 border border-white/[0.04] flex flex-col md:flex-row items-start md:items-center gap-4">
                        <div className="flex-1">
                            <p className="text-xs font-semibold text-white/60 mb-1">Quick Data Export</p>
                            <p className="text-[11px] text-white/25">Download all movement signups and contact messages as JSON — paste directly into Odoo import wizard or feed into your automation tool.</p>
                        </div>
