                ...
                <div className="max-w-5xl mx-auto space-y-16">
                    ...
                    <div>
                        ...
                        <div className="space-y-4">
                            {principles.map((p, i) => (
                                <motion.div key={i} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1 }} className="flex items-start gap-4 glass rounded-2xl p-5 border border-foreground/[0.04]">
                                    <span className="text-cyan-500/30 font-mono text-sm shrink-0 mt-0.5">{String(i + 1).padStart(2, '0')}</span>
                                    <p className="text-sm text-foreground/50 leading-relaxed">{p}</p>
                                </motion.div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="grid sm:grid-cols-2 gap-4">
                            {projects.map((proj, i) => (
                                <motion.a key={proj.name} href={proj.url} target="_blank" rel="noopener noreferrer" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1 }} className="glass rounded-2xl p-6 border border-foreground/[0.04] hover:border-cyan-500/20 transition-all duration-400 group flex items-start gap-4">
                                    <div className="w-10 h-10 rounded-xl glass border border-foreground/[0.06] flex items-center justify-center shrink-0">
                                        <proj.icon className="w-4 h-4 text-cyan-400/50 group-hover:text-cyan-400/80 transition-colors" />
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2 mb-1">
                                            <h3 className="text-sm font-semibold text-foreground/70 group-hover:text-foreground/90 transition-colors">{proj.name}</h3>
                                            <ExternalLink className="w-3 h-3 text-foreground/20 group-hover:text-cyan-400/60 transition-colors" />
                                        </div>
                                        <p className="text-xs text-foreground/30 leading-relaxed">{proj.desc}</p>
                                    </div>
                                </motion.a>
                            ))}
                        </div>
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="glass-strong rounded-3xl p-8 md:p-10 border border-cyan-500/10 text-center">
                        <Github className="w-10 h-10 text-foreground/30 mx-auto mb-4" />
                        <h3 className="text-xl font-bold text-foreground/70 mb-2" style={{ fontFamily: 'Outfit, Inter, sans-serif' }}>Coqui Cloud Dev Co.</h3>
                        <p className="text-sm text-foreground/30 mb-6 max-w-md mx-auto">The technical home of the hOS movement. Open source, community-driven, patient-first.</p>
                        <a href="https://coqui.cloud" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl glass border border-foreground/[0.08] text-sm text-foreground/50 hover:text-cyan-400 hover:border-cyan-500/30 transition-all">
