...
export default function BlogPost() {
    ...
    if (loading) {
        return (
            <div className="bg-obsidian min-h-screen text-foreground flex flex-col pt-32 px-4 items-center">
                <Navbar />
    ...
    if (!postId || !post) {
        return (
            <div className="bg-obsidian min-h-screen text-foreground flex flex-col">
                <Navbar />
                <div className="flex-1 flex flex-col items-center justify-center text-center px-4 pt-32">
                    <h1 className="text-3xl font-bold mb-4">Post Not Found</h1>
                    <p className="text-foreground/50 mb-8">The article you are looking for does not exist or has been removed.</p>
    ...
    return (
        <div className="bg-obsidian min-h-screen text-foreground">
            ...
            <article className="pt-40 pb-20 px-4 md:px-8 max-w-4xl mx-auto">
                <Link to="/Blog" className="inline-flex items-center gap-2 text-foreground/40 hover:text-foreground transition-colors mb-8 text-sm font-medium">
                    <ArrowLeft className="w-4 h-4" /> Back to Blog
                </Link>

                <div className="flex flex-wrap items-center gap-3 mb-6">
                    {post.category && (
                        <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-foreground/5 border border-foreground/10 text-xs font-medium text-foreground/70">
                ...
                <div className="prose prose-invert prose-cyan max-w-none prose-p:text-foreground/70 prose-p:leading-relaxed prose-headings:font-bold prose-headings:text-foreground/90 prose-a:text-cyan-400">
