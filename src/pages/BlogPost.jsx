...
const extractFirstMarkdownImage = (markdownText) => {
    if (!markdownText)
        return "https://humanos.foundation/patient-terminal-sandbox.png";
            ...
            if (data && data.contentUrl) {
                try {
                    ...
                    if (response.ok) {
                        ...
                        data.content =
                            "> *Error loading secure markdown file.*";
                    }
                } catch {
            ...
            <div className="bg-obsidian min-h-screen text-white flex flex-col">
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center px-4 pt-32">
                    ...
                    <p className="text-white/50 mb-8">
                        The article you are looking for does not exist or has
                        been removed.
                    </p>
                    <Link
                        to={createPageUrl("Blog")}
                        className="glow-btn px-6 py-3 rounded-xl bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 font-medium flex items-center gap-2"
                    >
    ...
    return (
        <div className="bg-obsidian min-h-screen text-white">
            <SEOMeta
                ...
                image={
                    post.image_url || extractFirstMarkdownImage(post.content)
                }
                type="article"
                article={{
            ...
            <article className="pt-40 pb-20 px-4 md:px-8 max-w-4xl mx-auto">
                <div className="flex items-center justify-between mb-8">
                    ...
                    <NativeShareWidget
                        ...
                        text={
                            post.excerpt ||
                            `Read ${post.title} on the Humanos Foundation Blog.`
                        }
                        url={`https://humanos.foundation/research/${post.id}`}
                    />
                </div>
                            ...
                            <div className="w-full">
                                <div className="w-full bg-obsidian-light/30 border border-white/5 rounded-xl p-6 text-center h-24 mb-3 flex items-center justify-center">
                                    <p className="text-sm text-white/50 font-medium">
                                        On-page comments are temporarily locked.
                                    </p>
                                </div>
                                <div className="flex justify-between items-center">
                                    <span className="text-[10px] uppercase tracking-wider text-white/30 font-medium">
                                        Secure API Defrosting in Process
                                    </span>
                                    <a
                                        href="https://discord.gg/humanos"
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="glow-btn px-6 py-2.5 rounded-xl bg-blue-500/10 border border-blue-500/30 text-blue-400 font-bold text-sm hover:bg-blue-500/20 hover:shadow-[0_0_20px_rgba(59,130,246,0.3)] transition-all flex items-center gap-2"
                                    >
                                        Discuss on Discord{" "}
